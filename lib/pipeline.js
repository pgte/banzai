...
function Pipeline(name, options) {
  ...
  function toState(doc, stateDoc, state, done) {
    ...
    try {
      ...
      // Save state doc id in doc._states
      (function() {
        var stateDocId = stateDoc._id || stateDoc._id;
        if (stateDocId) {
          if (! doc._states) { doc.states = []; }
          if (doc._states.indexOf(stateDocId) === -1) {
            doc._states.push(stateDocId);
          }
        }
      }());
  ...
  function toErrorState(doc, stateDoc, err, fromError) {
    ...
    try {
      that.notifier.emit('error', err, doc, stateDoc);
    } catch(_err) {
      error('Error notifying of error: ' + _err.message + '. Stack: ' + inspect(_err.stack));
    }
  }
  ...
  function workerForward(stateDoc, stateName, done) {
    ...
    try {
      ...
      loadDoc(stateDoc.doc_id, context, function(err, doc) {
        ...
        if (err) { return done(err); }
        handleStateEntry(doc, stateDoc, stateName, function(err) {
  ...
  function on(stateName, handler, options) {
    ...
    queue.pop(jobName(stateName), function(job, done) {
      workerForward(job, stateName, function(err) {
        if (err) { error(err); }
        done();
      });
    });
