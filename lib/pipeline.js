...
function Pipeline(name, options) {
  ...
  function jobBackName(fromState, toState) {
    return 'banzai-__BACK__-' + fromState + '-' + toState;
  }
  ...
  function handleStateEntry(doc, stateDoc, stateName, done) {
    ...
    function noMoreStates() {
      ...
      saveState(doc, stateDoc, function(err) {
        if (err) { return done(err); }
  ...
  function workerBack(stateDoc, done) {
    var transitions
      , oldTransition
      , state;
    ...
    oldTransition = transitions.splice(transitions.length - 1, 1);
    if (! oldTransition.old_rev) { return done(new Error('Transition does not say which revision to roll back to')); }
    state = states[oldTransition.from];
    if (! state) { return done(new Error('State not found: ' + oldTransition.from)); }
    state.backToThis(oldTransition.to, function(err) {
      if (err) { return done(err); }
      docStore.backToRevision(stateDoc.doc_id, oldTransition.old_rev, function(err, newDoc) {
        if (err) { return done(err); }
        saveState(newDoc, stateDoc, done);
      });
    });
  }
  ...
  function back(id, from, to, done) {
    ...
    loadState(id, { log: verbose }, function(err, stateDoc) {
      if (err) { return done(err); }
      queue.push(jobBackName(from, to), stateDoc, done);
    });
  }
